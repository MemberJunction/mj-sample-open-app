...
| npm scope `@mj-sample-app/*` | all `packages/*/package.json` names + cross-deps, `mj-app.json` `packages` block, root `package.json` `build:packages`/`test` filters + `pnpm.overrides`, `.changeset/config.json` `fixed`, `.github/workflows/*` + `.github/scripts/*` greps, `mj.config.cjs` `entityPackageName` | `grep -r "mj-sample-app" .` finds them all |
